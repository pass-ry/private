...
var (
	constDeliverQueue = "liepin_push_parser"
)
...
func delive(conn redis.Client, ac *account.Account, r *result.Result) error {
	...
	if len(dfsM["img"]) > 0 && err != nil {
		return errors.Wrap(err, "delive fdfs.GetConstClient.Get(img)")
	}
	dfsJ, err := fdfs.GetConstClient().Get(dfsM["json"])
	if len(dfsM["json"]) > 0 && err != nil {
		return errors.Wrap(err, "delive fdfs.GetConstClient.Get(json)")
	}
	pdfJ, err := fdfs.GetConstClient().Get(dfsM["pdf"])
	if len(dfsM["pdf"]) > 0 && err != nil {
		return errors.Wrap(err, "delive fdfs.GetConstClient.Get(pdf)")
	}
	dfsH, err := fdfs.GetConstClient().Get(dfsM["html"])
	if len(dfsM["html"]) > 0 && err != nil {
		return errors.Wrap(err, "delive fdfs.GetConstClient.Get(html)")
	}
	...
	if len(pdfJ) > 0 {
		dfsM["pdf"] = string(pdfJ)
	}
	if len(dfsH) > 0 {
		dfsM["html"] = string(dfsH)
	}
