...
func GetResume(req *handler.Request, rsp *handler.Response) error {
	...
	if err != nil {
		return fmt.Errorf("liepin GetResume req.Unmarshal err=%v", err)
	}

	if len(params.Username) == 0 {
		log.Error("liepin GetResume username is null")
		return handler.WrapError(fmt.Errorf("liepin GetResume username is null"), -1, "username is null")
	}

	tobRes, err := getResumeRes(&params)
	if err != nil {
		return handler.WrapError(err, -1, "liepin get resume is failed")
	}
...
func getResumeRes(params *tobGetResumeListReq) (*tobGetResumeListRsp, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("liepin getResumeRes getTableName err=%v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("liepin getResumeRes SELECT error=%v", err)
	}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("liepin getResumeRes rows.Err error=%v", err)
	}
...
func getTableName(name string) (string, int64, error) {
	var id int64
	err := mysql.GetConstConn().QueryRow(`SELECT id FROM liepin where username=? limit 1`, name).Scan(
		&id)
	if err != nil {
		return "", 0, fmt.Errorf("liepin resume getUserNameID err=%v", err)
	}

	return fmt.Sprintf("liepin_%d", id%8), id, nil
}
