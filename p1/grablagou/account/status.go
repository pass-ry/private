...
func (ac *Account) Push(code, source int) error {
	...
	if err != nil {
		return errors.Wrap(err, "lagou Account-Push Marshal")
	}

	conn, err := redis.GetConstClient()
	if err != nil {
		return errors.Wrap(err, "lagouAccount-Push redis.GetConstClient")

	}
	defer conn.Close()
	_, err = conn.Do("LPUSH", "lagou_account_forward_queue", pushMsg)
	if err != nil {
		return errors.Wrap(err, "lagouAccount-Push LPUSH")
	}
