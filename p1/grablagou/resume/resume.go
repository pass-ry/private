...
func GetResume(req *handler.Request, rsp *handler.Response) error {
	...
	if err != nil {
		return handler.WrapError(err, -1, "lagou GetResume req.Unmarshal failed")
	}

	if len(params.Username) == 0 {
		log.Error("lagou GetResume username is null")
		return handler.WrapError(fmt.Errorf("lagou GetResume username is null"), -1, "username is null")
	}

	tobRes, err := getResumeRes(&params)
	if err != nil {
		return handler.WrapError(err, -1, "lagou get resume is failed")
	}
...
func getResumeRes(params *tobGetResumeListReq) (*tobGetResumeListRsp, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("lagou getResumeRes getTableName err=%v", err)
	}
	...
	// 构造查询SQL
	sql := fmt.Sprintf("SELECT id, account_id, cv_id, jd_id, receive_time, cv_name, jd_name, status, msg, content_dfs, icdc_id, is_deleted, updated_at, created_at FROM %s where account_id=%d and receive_time>='%s' and receive_time<='%s' limit %d, %d",
	...
	if err != nil {
		return nil, fmt.Errorf("lagou getResumeRes SELECT error=%v", err)
	}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("lagou getResumeRes rows.Err error=%v", err)
	}
...
func getTableName(name string) (string, int64, error) {
	var id int64
	err := mysql.GetConstConn().QueryRow(`SELECT id FROM lagou where username=? limit 1`, name).Scan(
		&id)
	if err != nil {
		return "", 0, fmt.Errorf("lagou resume getUserNameID err=%v", err)
	}

	return fmt.Sprintf("lagou_%d", id%8), id, nil
}
